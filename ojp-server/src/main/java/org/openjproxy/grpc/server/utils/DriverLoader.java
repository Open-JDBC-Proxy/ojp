...
/**
 * Utility class for loading JDBC drivers from a configurable directory at
 * runtime.
 * This allows customers to add proprietary drivers without recompiling OJP
 * Server.
 */
@Slf4j
@UtilityClass
public class DriverLoader {
     ...
     * 
     * @param driversPath Path to the directory containing external library JAR
     *                    files
     * @return true if loading was successful (even if no JARs found), false on
     *         error
     */
    public boolean loadDriversFromPath(String driversPath) {
        ...
        if (!Files.exists(driverDir)) {
            ...
            log.info(
                    "No external libraries will be loaded. To add proprietary drivers, create the directory and place JAR files there.");
            return true;
        }
     ...
     * This is necessary because DriverManager only accepts drivers loaded by the
     * system class loader
     * or its parent. This shim delegates all calls to the actual driver.
     * 
     * Package-private visibility allows DriverUtils to check if drivers are
     * properly loaded
