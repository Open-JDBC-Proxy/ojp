...
class PerDatasourceSlowQuerySegregationTest {
        ...
        void testManagerRetrievalForExistingConnection() throws Exception {
                ...
                Field managersField = StatementServiceImpl.class.getDeclaredField("slowQuerySegregationManagers");
                managersField.setAccessible(true);
                Map<String, SlowQuerySegregationManager> managers = (Map<String, SlowQuerySegregationManager>) managersField
                                .get(statementService);

                SlowQuerySegregationManager manager = managers.get(connHash);
        ...
        void testFallbackManagerForNonExistentConnection() throws Exception {
                // Fallback is now handled in the Action classes, but we can verify the map
                // state
                Field managersField = StatementServiceImpl.class.getDeclaredField("slowQuerySegregationManagers");
                managersField.setAccessible(true);
                Map<String, SlowQuerySegregationManager> managers = (Map<String, SlowQuerySegregationManager>) managersField
                ...
                SlowQuerySegregationManager manager = managers.get("non-existent-hash");

                assertNull(manager,
                                "Should return null for non-existent connection hash when accessing the map directly");
        }
