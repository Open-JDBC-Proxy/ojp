...
class Db2StatementExtensiveTests {
    ...
    void testExecuteUpdate(String driverClass, String url, String user, String password) throws Exception {
        this.setUp(driverClass, url, user, password);
        int rows = statement
                .executeUpdate("UPDATE DB2INST1.db2_statement_test SET name = 'Updated Alice' WHERE id = 1");
        assertEquals(1, rows);
    ...
    void testExecuteAfterCloseThrows(String driverClass, String url, String user, String password) throws Exception {
        ...
        assertThrows(SQLException.class,
                () -> statement.executeUpdate("UPDATE DB2INST1.db2_statement_test SET name = 'fail' WHERE id = 1"));
        assertThrows(SQLException.class, () -> statement
                .addBatch("INSERT INTO DB2INST1.db2_statement_test (id, name) VALUES (99, 'ShouldFail')"));
    }
    ...
    void testLargeMethodsDefault(String driverClass, String url, String user, String password) throws Exception {
        ...
        assertDoesNotThrow(
                () -> statement.executeLargeUpdate("UPDATE DB2INST1.db2_statement_test SET name = 'x' WHERE id = 1"));
        assertDoesNotThrow(() -> statement.executeLargeUpdate(
                "UPDATE DB2INST1.db2_statement_test SET name = 'x' WHERE id = 1", Statement.RETURN_GENERATED_KEYS));
        assertDoesNotThrow(() -> statement.executeLargeUpdate(
                "UPDATE DB2INST1.db2_statement_test SET name = 'x' WHERE id = 1", new String[] { "id" }));
        assertDoesNotThrow(() -> statement
                .executeLargeUpdate("UPDATE DB2INST1.db2_statement_test SET name = 'x' WHERE id = 1", new int[] { 1 }));
