 ...
 * This ensures that all tests share the same SQL Server instance to improve
 * test performance
...
public class SQLServerTestContainer {
    ...
    public static MSSQLServerContainer<?> getInstance() {
        // Fast-path: if container already created and running, return it without
        // locking
        ...
        try {
            ...
            if (!isStarted) {
                ...
                try {
                    ...
                    // Do not fail tests on init best-effort; XA tests will fail with clearer
                    // message if needed
                    System.err.println("[SQLServerTestContainer] Warning: Failed to install XA stored procedures: "
                            + e.getMessage());
                }
    ...
    /**
     * Installs Microsoft SQL Server XA stored procedures required for XA
     * transactions.
    ...
    private static void installXaStoredProcedures() throws Exception {
        // Path to sqlcmd inside the container (tools18 has TLS defaults compatible with
        // -C switch)
    ...
    /**
     * Creates defaultdb database and testuser with db_owner in defaultdb to align
     * with existing tests.
     */
    private static void createDefaultDbAndUser() throws Exception {
        ...
        String[] createUser = new String[] { sqlcmd, "-S", "localhost", "-U", saUser, "-P", saPassword, "-d",
                "defaultdb", "-C", "-Q",
    ...
    /**
     * Grants execute permissions on XA extended stored procedures to testuser in
     * master DB and adds to SqlJDBCXAUser role.
     */
    private static void grantXaPermissionsToTestUser() throws Exception {
        ...
        String[] grantCmd = new String[] { sqlcmd, "-S", "localhost", "-U", saUser, "-P", saPassword, "-d", "master",
                "-C", "-Q", grantScript };
