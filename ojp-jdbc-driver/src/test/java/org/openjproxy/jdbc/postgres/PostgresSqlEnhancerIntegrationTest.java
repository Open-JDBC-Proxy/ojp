 ...
 * 1. Creates test database with realistic data (Regions: 500, Customers: 25000,
 * Orders: 50000)
 ...
 * - Schema metadata is loaded from PostgreSQL (3 tables: regions, customers,
 * orders)
...
public class PostgresSqlEnhancerIntegrationTest {
    ...
    // While optimization may be limited due to PostgreSQL/Calcite type system
    // differences,
    ...
    //
    // For production use, SQL enhancement works best with simpler standard SQL
    // queries
    // or when comprehensive type mapping is configured between the database and
    // Calcite.
    private static final String TEST_QUERY = "SELECT region_name, country\n" +
            "FROM regions\n" +
            "WHERE country = 'USA'\n" +
    ...
    private static void setupTestData() throws Exception {
        ...
        try (Connection conn = PG_DRIVER.connect(pgUrl, props);
            ...
            // Pattern uses lookahead to match semicolon followed by whitespace and
            // newline/end without consuming them
            ...
            for (int i = 0; i < statements.length; i++) {
                ...
                // Skip comment-only statements or statements that become empty after removing
                // comments
                if (finalSql.isEmpty()) {
                    continue;
                }
    ...
    public void testSqlEnhancerPerformance() throws Exception {
        ...
        // The first call will actually execute the original query as per no cache
        // exists.
